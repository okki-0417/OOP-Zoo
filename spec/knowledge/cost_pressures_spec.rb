# frozen_string_literal: true

require 'spec_helper'

# 継続コストと拡大投資の知識（バックログ）。
#
# 反・暴走の圧力(b): 拡大はコストとリスクを連れてくる。大きくするほど脆くなり、
# 「何もしないと必ず傾く＝再投資し続ける緊張」を生む。
# (a) 収容力の上限も投資対象としてここに置く。
# 本文のない `it` は未設計のバックログ(pending)。
RSpec.describe '継続コストと拡大投資' do
  describe '収容力の拡張(投資)' do
    it '来園の収容力は投資で拡張できること'
    it '収容力を超える需要は取りこぼされること'
  end

  describe '展示のリニューアル(投資)' do
    it '展示は古びると集客が落ち、リニューアル投資で回復すること'
  end

  describe '獣医費(継続)' do
    it '高齢・病気の個体ほど獣医費が継続的にかさむこと'
  end

  describe '施設の維持・減価(継続)' do
    it '維持を怠ると施設が劣化し、後の修繕費が大きくなること'
  end

  describe '福祉維持費(継続)' do
    it 'エンリッチメント等の福祉維持費を削ると福祉が下がり、集客に響くこと'
  end

  describe '人件費' do
    it '職員給与は飼育負荷(頭数・専門)に応じて増えること(現状は定額)'
  end
end

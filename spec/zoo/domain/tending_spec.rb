    ...
    RSpec.describe Tending do
      ...
      describe '#assignment_violation!' do
        it 'occupants 未指定なら専門に関わらず例外を出さないこと' do
          tending = described_class.new(keeper: keeper, enclosure: enclosure)
          expect { tending.assignment_violation! }.not_to raise_error
        end

        it '専門外の綱が混ざると TendingNotAllowed を綱ラベル付きで出すこと' do
          tending = described_class.new(keeper: keeper, enclosure: enclosure, occupants: [lion, penguin])
          expect { tending.assignment_violation! }
            .to raise_error(Errors::TendingNotAllowed, /田中.*サバンナ.*鳥類/)
        end
        ...
        it 'keepers に同一 id の飼育員がいれば二重配属として TendingNotAllowed を出すこと' do
          tending = described_class.new(keeper: keeper, enclosure: enclosure, keepers: [keeper])
          expect { tending.assignment_violation! }
            .to raise_error(Errors::TendingNotAllowed, /田中.*すでに.*サバンナ/)
        end

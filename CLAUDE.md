...
- 集約 = エンティティ = 永続化の単位
- 集約に属するVOは、集約ごとのディレクトリに置く（例: `app/domain/animal.rb` の VO は `app/domain/animal/` に置く）
- アプリケーションサービスなどからドメインロジックを呼ぶ場合は、集約のルートを経由して呼ぶ（例: `Animal#sex_label` など）
- ある集約は、別の集約のことを知ってはいけない。集約同士の間で起こるイベント（例: 動物の収容）に関するロジックやルールは、ドメインサービスに置く（例: `app/domain/breeding.rb`）

...
### あるリソースを表す集約は、別の集約のことを知ってはいけない

- 集約同士の間で起こるイベント（例: 動物の収容）に関するロジックやルールは、イベント用のエンティティやサービスを作成する
- 例: `Animal` と `Enclosure` はお互いを知らない。収容に関するルールは `EnclosureAssignment` などのエンティティやサービスに書く
